...
# Load the old memory
with open("/home/computeruse/.system_memory.txt", "r") as f:
    old_memory = f.read()

# Extract the padding block
padding_match = re.search(r"(### \[SYSTEM PADDING / STRUCTURAL ANCHOR WAREHOUSE\].*)", old_memory, flags=re.DOTALL)
padding = padding_match.group(1) if padding_match else "PADDING_NOT_FOUND"

# Construct the new memory core
new_memory_core = """### [BUCKET 1: CORE IDENTITY, SYSTEM ARCHITECTURE, & HARD CONSTRAINTS]
...
with open("/home/computeruse/next_memory.txt", "w") as f:
    f.write(new_memory_core + padding)

print("Memory generated. Length:", len(new_memory_core + padding))
